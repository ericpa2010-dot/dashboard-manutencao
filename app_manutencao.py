...
def load_and_process_data():
    ...
    def sanitizar_tecnico(r):
        ...
        if tec_raw == "" or tec_raw.lower() in ["nan", "none", "não atribuído", "-"]:
            if pd.notna(dt_ab) and dt_ab < DATA_CORTE_TECNICO:
...
with tab_dash:
    ...
    if df_calc.empty:
        ...
        if opcao_periodo == "Últimos 30 dias":
            limite_dt_tec = agora_naive_geral - pd.Timedelta(days=30)
            df_tec_base = df_concluidos[df_concluidos["dt_abertura"] >= limite_dt_tec].copy()
        elif opcao_periodo == "Últimos 90 dias":
            limite_dt_tec = agora_naive_geral - pd.Timedelta(days=90)
            df_tec_base = df_concluidos[df_concluidos["dt_abertura"] >= limite_dt_tec].copy()
        elif opcao_periodo == "Este Mês":
            limite_dt_tec = agora_naive_geral.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            df_tec_base = df_concluidos[df_concluidos["dt_abertura"] >= limite_dt_tec].copy()
        elif opcao_periodo == "Este Ano":
            limite_dt_tec = agora_naive_geral.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)
            df_tec_base = df_concluidos[df_concluidos["dt_abertura"] >= limite_dt_tec].copy()
        else:
            df_tec_base = df_concluidos.copy()

        if not df_tec_base.empty:
            df_tec_base["Tecnico_Agrupado"] = df_tec_base["Tecnico_Clean"].apply(lambda x: "Eric" if x in ["Não atribuído", "Eric (Histórico Geral)"] else x)
            
            tec_stats = df_tec_base.groupby("Tecnico_Agrupado").agg(
        ...
        if df_calc.empty:
            ...
            for _, row in df_calc.iterrows():
                ...
                lista_gestao.append({
                    ...
                    "Tempo / SLA": tempo_str,
                    "Situação SLA": sit_str,
                    "Técnico Responsável": tec_val if tec_val != "Não atribuído" else "Eric"
                })
        ...
        if mask_num.any():
            ...
            with st.form("form_atualizacao"):
                ...
                if btn_salvar:
                    ...
                    st.success(f"Chamado Nº {num_chamado} atualizado com sucesso!")
                    st.cache_data.clear()